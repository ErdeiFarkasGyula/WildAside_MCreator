...
public class WildasideModVariables {
	...
	public static void init(FMLCommonSetupEvent event) {
		WildasideMod.addNetworkMessage(SavedDataSyncMessage.class, SavedDataSyncMessage::buffer, SavedDataSyncMessage::new,
				SavedDataSyncMessage::handler);
		WildasideMod.addNetworkMessage(PlayerVariablesSyncMessage.class, PlayerVariablesSyncMessage::buffer, PlayerVariablesSyncMessage::new,
	...
	public static class EventBusVariableHandlers {
		...
		public static void clonePlayer(PlayerEvent.Clone event) {
			...
			PlayerVariables clone = ((PlayerVariables) event.getEntity().getCapability(PLAYER_VARIABLES_CAPABILITY, null)
					.orElse(new PlayerVariables()));
			if (!event.isWasDeath()) {
				clone.currentPage = original.currentPage;
			}
		}
		...
		public static void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {
			if (!event.getPlayer().level.isClientSide()) {
				...
				if (mapdata != null)
					WildasideMod.PACKET_HANDLER.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer) event.getPlayer()),
	...
	public static class WorldVariables extends SavedData {
		...
		public void read(CompoundTag nbt) {
			allPages = nbt.getDouble("allPages");
		}

		@Override
		public CompoundTag save(CompoundTag nbt) {
			nbt.putDouble("allPages", allPages);
			return nbt;
		}
	...
	public static class PlayerVariables {
		...
		public Tag writeNBT() {
			...
			nbt.putDouble("currentPage", currentPage);
			return nbt;
		}

		public void readNBT(Tag Tag) {
			CompoundTag nbt = (CompoundTag) Tag;
			currentPage = nbt.getDouble("currentPage");
		}
	}
			...
			context.enqueueWork(() -> {
				if (!context.getDirection().getReceptionSide().isServer()) {
					PlayerVariables variables = ((PlayerVariables) Minecraft.getInstance().player.getCapability(PLAYER_VARIABLES_CAPABILITY, null)
							.orElse(new PlayerVariables()));
					variables.currentPage = message.data.currentPage;
				}
			});
			context.setPacketHandled(true);
		}
