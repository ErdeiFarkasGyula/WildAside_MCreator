...
public class VibrionMonsterEntity extends Monster implements IAnimatable {
	public static final EntityDataAccessor<Boolean> SHOOT = SynchedEntityData.defineId(VibrionMonsterEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(VibrionMonsterEntity.class, EntityDataSerializers.STRING);
	private AnimationFactory factory = GeckoLibUtil.createFactory(this);
	private boolean swinging;
	private boolean lastloop;
	private long lastSwing;
	public String animationprocedure = "empty";
	private static final Set<ResourceLocation> SPAWN_BIOMES = Set.of(new ResourceLocation("wildaside:vibrion_hive"));
	...
	public VibrionMonsterEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(WildasideModEntities.VIBRION_MONSTER.get(), world);
	}

	public VibrionMonsterEntity(EntityType<VibrionMonsterEntity> type, Level world) {
		super(type, world);
		xpReward = 10;
		setNoAi(false);
	}
	...
	private <E extends IAnimatable> PlayState movementPredicate(AnimationEvent<E> event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.vibrion_monster.walk", EDefaultLoopTypes.LOOP));
				return PlayState.CONTINUE;
			}
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.vibrion_monster.idle", EDefaultLoopTypes.LOOP));
			return PlayState.CONTINUE;
		}
	...
	private <E extends IAnimatable> PlayState attackingPredicate(AnimationEvent<E> event) {
		...
		if (this.swinging && event.getController().getAnimationState().equals(software.bernie.geckolib3.core.AnimationState.Stopped)) {
			...
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.vibrion_monster.attack", EDefaultLoopTypes.PLAY_ONCE));
			return PlayState.CONTINUE;
		}
		return PlayState.CONTINUE;
	}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(VibrionMonsterEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
